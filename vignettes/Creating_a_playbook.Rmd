...
vignette: >
  %\VignetteIndexEntry{Writing_a_playbook}
...
Alternatively, you can add custom objects to the ggulti plot values object:

```{r, message=FALSE, warning=FALSE}
ggulti_plot_values$obj_cols["Ref"] <- "black"
ggulti_plot_values$obj_shapes["Ref"] <- 18
ggulti_plot_values$obj_sizes["Ref"] <- 2 

plot_play(pitch,object_list = custom)
```

By customising objects, you can specifiy specific teams or even colour positions in a zone differently.

```{r, message=FALSE, warning=FALSE}
zone_positions <- c("Cup" = "blue", "Wing" = "red", "Mid" = "yellow", "Deep" = "green")
ggulti_plot_values$obj_cols <- append(ggulti_plot_values$obj_cols, zone_positions)

zone <- list(
  pitch_object("Cup", show=T, x=c(14), y=c(25)),
  pitch_object("Cup", show=T, x=c(18), y=c(28)),
  pitch_object("Cup", show=T, x=c(22), y=c(25)),
  pitch_object("Wing", show=T, x=c(10), y=c(40)),
  pitch_object("Wing", show=T, x=c(27), y=c(40)),
  pitch_object("Mid", show=T, x=c(18), y=c(38)),
  pitch_object("Deep", show=T, x=c(18), y=c(60))
)

plot_play(ggpitch(endzone_fill="darkgrey"),object_list = zone)
